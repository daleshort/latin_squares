...
def solve_sudoku(sizes, grid):
    ...
    for size in sizes:
        ...
        # N = R * C
        X += [("r%s" % str(size), rn)
              for rn in product(range(N), range(1, N + 1))]
        for r, c, n in product(range(N), range(N), range(1, N + 1)):
...
def MakePuzzle(WhatIsLatin: Board = Board((1, 6), (6, 1), [(2, 3), (3, 2)])) -> str:
    ...
    TestPuzzle = solution = list(
        islice(solve_sudoku(LatinList, TestPuzzle), WhichSol, WhichSol+1))[0]
    numOfSolutions = 1
    ...
    CorrectAnswer = list(solve_sudoku(
        LatinList, copy.deepcopy(UniquePuzzle)))[0]
...
>>>>>>> 25a234ffcb74f68638accd26a8b4cd3aa13834d0
